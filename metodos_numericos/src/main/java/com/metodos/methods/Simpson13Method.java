...
public class Simpson13Method implements IntegrationMethod {
    ...
    public IntegrationResult calculate(MathParser parser, double a, double b, int n) throws IllegalArgumentException, ArithmeticException {
        ...
        // Apply Simpson's 1/3 Rule and gather terms
        double sumTerms = 0;
        ...
        steps.add("   Integral ≈ (h / 3) * Suma");
        steps.add(String.format(Locale.US, "   Integral ≈ (%.8f / 3) * %.8f", h, sumTerms));
        steps.add(String.format(Locale.US, "   Integral ≈ %.8f * %.8f", (h / 3.0), sumTerms));
